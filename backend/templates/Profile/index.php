        ...
        <div class="flex <?= $isMobileView ? 'flex-col items-center text-center' : 'items-start' ?> gap-6">
            ...
            <div class="flex-shrink-0">
                <?php if (!empty($user->profile_photo_path)): ?>
                    <img src="<?= h($user->profile_photo_path) ?>" 
                         alt="<?= h($user->username) ?>" 
                         class="<?= $isMobileView ? 'w-24 h-24' : 'w-32 h-32' ?> rounded-full object-cover border-2 border-gray-200">
                <?php else: ?>
                    <div class="<?= $isMobileView ? 'w-24 h-24' : 'w-32 h-32' ?> rounded-full bg-gradient-to-br from-blue-400 to-purple-500 flex items-center justify-center text-white <?= $isMobileView ? 'text-3xl' : 'text-5xl' ?> font-bold">
            ...
            <div class="flex-1">
                <div class="flex <?= $isMobileView ? 'flex-col' : 'items-center' ?> gap-4 mb-1">
                    <h1 class="text-2xl font-semibold text-gray-900"><?= h($user->username) ?></h1>
                ...
                <div class="text-gray-600 font-medium m">
                    <div class="font-semibold"><?= h($user->full_name) ?></div>
                </div>
...
(function() {
            ...
            <div class="flex items-center justify-between p-2 hover:bg-gray-50 rounded-lg">
                <div class="flex items-center space-x-3 flex-1 min-w-0">
                    ${user.profile_photo_path 
                        ? `<img src="${escapeHtml(user.profile_photo_path)}" class="w-12 h-12 rounded-full object-cover flex-shrink-0">`
                        : `<div class="w-12 h-12 rounded-full bg-blue-500 flex items-center justify-center text-white font-semibold flex-shrink-0">${user.username.charAt(0).toUpperCase()}</div>`
                    }
                    <div class="min-w-0 flex-1">
                        <a href="/profile/${escapeHtml(user.username)}" class="profile-link text-sm font-semibold text-gray-900 hover:underline block truncate" data-username="${escapeHtml(user.username)}">
                            ${escapeHtml(user.full_name)}
                        </a>
                        <p class="text-xs text-gray-500 truncate">@${escapeHtml(user.username)}</p>
                    </div>
                </div>
                ${user.is_following 
                    ? `<button class="unfollow-user-btn px-4 py-1.5 text-xs font-semibold border border-gray-300 rounded-lg hover:bg-red-50 hover:border-red-300 hover:text-red-600 transition-colors" data-user-id="${user.id}" data-username="${escapeHtml(user.username)}">Unfollow</button>`
                    : `<button class="follow-user-btn px-4 py-1.5 text-xs font-semibold bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition-colors" data-user-id="${user.id}" data-username="${escapeHtml(user.username)}">Follow</button>`
                }
            </div>
        `).join('');
    }
    ...
    // Intercept profile link clicks in modal to load via AJAX
    document.getElementById('followers-modal').addEventListener('click', function(e) {
        ...
        if (profileLink) {
            ...
            console.log('Profile link clicked:', username, 'URL:', url);
            console.log('loadMiddleColumn available:', typeof window.loadMiddleColumn);
            
            // Close modal
            document.getElementById('followers-modal').classList.add('hidden');
            
            // Load profile in middle column if function exists
            if (typeof window.loadMiddleColumn === 'function') {
                console.log('Loading via AJAX');
                window.loadMiddleColumn(url);
            } else {
                console.log('Fallback to full page load');
